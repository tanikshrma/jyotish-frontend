...
## 7b. Kundli PDF delivery (paid export)

On successful payment the server:

1. renders the report via VedicAstro `pdf/horoscope-queue` (`pdf_type=large`);
2. uploads it to the **Prospect IQ media library**, which returns a permanent,
   public CDN URL — no local disk, so this works on Vercel and self-hosted alike;
3. emails it to the customer through Prospect IQ with the PDF attached;
4. returns the URL so the browser downloads **and** opens it.

VedicAstro's own link expires after ~2 hours, which is why the file is
re-hosted immediately rather than sent to the customer directly.

If PDF generation fails (e.g. credits exhausted) the customer has already paid,
so the UI falls back to the printable view and shows the reason.

Branding on the report's last page comes from `PDF_COMPANY_NAME`, `PDF_ADDRESS`,
