 ...
 * Returns: { verified: true, payment_id, order_id, whatsappAdminUrl, whatsappCustomerUrl } on a signature match.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 1. Send Email Receipt via Resend API if configured
    ...
    // 2. Sync Payment & Lead details to ProspectIQ CRM
    if (token) {
      try {
    ...
    // 3. Format WhatsApp receipt URLs safely
